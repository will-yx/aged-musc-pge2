...
@njit
def make_valsi(hs, ins):
  return np.sum(hs * ins, axis=2).astype(np.float64)

t0 = timer()
valsi = make_valsi(f['hyp_scores'][:], f['input_seqs'][:])
tB += timer()-t0
...
for i in iterator:
    ...
    #t0 = timer()
    #vals = np.sum(f['hyp_scores'][i]*f['input_seqs'][i], axis=1).astype('<f8')[cur_end-regions[i][1]:]
    #tA += timer()-t0
    t0 = timer()
    vals = valsi[i][cur_end-regions[i][1]:]
    tB += timer()-t0
